...
### The signal demo (durable human-in-the-loop approval)

```bash
# Submit a doc for approval. The run blocks on waitForSignal("approve") and
# parks as 'waiting' — no process needs to stay alive during the wait.
npm run start approvalFlow '{"orderId":"INV-42"}'
#   -> "[submit] INV-42 submitted; waiting for approval…"
npm run cli list                      # -> status: waiting

# Deliver the decision from anywhere (another terminal, minutes or days later).
# The run wakes, runs finalize, and completes.
npm run cli signal <workflow_id> approve '{"approved":true,"by":"alice"}'
#   -> "[finalize] INV-42 APPROVED by alice"  ·  status: completed
```

A signal delivered **before** the run reaches its wait is buffered in the log and
consumed when the wait is reached, so there's no lost-signal race. The wait is
durable: kill the process while `waiting` and the run resumes on the next signal,
without re-running `submit`.
...
npm run cli signal <id> <name> [jsonVal]    deliver an external signal to a run
...
- waiting on an external event (an approval, a webhook, another workflow) → use
  **`ctx.waitForSignal(name)`** (durable wait; see "Signals" below)
...
`ctx.waitForSignal(name)` durably blocks a workflow on an event delivered from
**outside** — a human approval, a webhook, another process. Delivery
(`runtime.sendSignal`, or `npm run cli signal`) appends a `SIGNAL_RECEIVED` to
the log; the wait consumes the earliest matching one and records a
`SIGNAL_CONSUMED` at its `seq`, so replay returns the identical payload. Three
properties fall out of doing it through the log:

- **Durable.** A run blocked on a signal is parked as `waiting` with no process
  alive. It resumes when a signal is delivered — seconds or days later.
- **No lost-signal race.** A signal delivered *before* the workflow reaches the
  wait is buffered in the log and consumed once the wait is reached. Order is by
  append `id`, and each `SIGNAL_RECEIVED` binds to at most one wait.
- **Crash-safe delivery.** Appending the signal and flipping `waiting → running`
  happen in one transaction, so a crash between the two is impossible; boot
  `recover()` then advances the now-`running` run. Signals are event-driven, so
  unlike timers they need no scheduler poll.
...
| `seq` | deterministic call index within the run (0,1,2…; `-1` lifecycle markers, `-2` signal deliveries) |
| `type` | `WORKFLOW_STARTED` · `ACTIVITY_COMPLETED` · `ACTIVITY_FAILED` · `TIMER_STARTED` · `TIMER_FIRED` · `SIGNAL_RECEIVED` · `SIGNAL_CONSUMED` · `WORKFLOW_COMPLETED` · `WORKFLOW_FAILED` |
...
A run blocked on a signal sits in `status='waiting'` until a delivery flips it back to `running`.
...
durable-engine/
  src/
    ...
    context.ts        WorkflowContext: activity / now / random / sleep / waitForSignal (the heart)
    ...
    workflows/
      ...
      approvalFlow.ts submit -> waitForSignal -> finalize (signal demo)
    cli.ts            start / recover / signal / history / status / list / worker
...
- Waiting workflows are **not** recovered directly either — they resume only when
  a signal is delivered, which flips them to `running` (covered by `recover()` if
  the process then dies before replay) and consumes the buffered `SIGNAL_RECEIVED`.
